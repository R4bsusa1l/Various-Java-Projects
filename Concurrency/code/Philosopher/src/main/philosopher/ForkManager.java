...
class ForkManager {
    ...
    private final Condition bothForksAreFree;
    ...
    public ForkManager(int numForks, int delayTime) {
        ...
        this.bothForksAreFree = managerLock.newCondition();
     ...
     */
    public synchronized void acquireForks(int philosopherId) throws InterruptedException {
        boolean forksArePaired = false;
        try {
            managerLock.lock();

            while (!forksArePaired) {
                if (leftFork(philosopherId).state.equals(FREE) && rightFork(philosopherId).state.equals(FREE)) {
                    leftFork(philosopherId).acquire(philosopherId);
                    forkDelay();
                    rightFork(philosopherId).acquire(philosopherId);
                    forksArePaired = true;
                    bothForksAreFree.signal();
                } else {
                    bothForksAreFree.await();
                }
            }
        }finally {
            managerLock.unlock();
    ...
    /**
     * Compute the number of forks taken
     * @return the number of forks taken
     */
    public int computeNumberOfForksTaken() {
        try {
            managerLock.lock();
            return Arrays.stream(forks).mapToInt(fork -> fork.state != FREE ? 1 : 0).sum();
        } finally {
            managerLock.unlock();
        }
    }
    ...
    private static class Fork {
        ...
        @Override
        public String toString() {
            return "Fork { id=%d, state=%s, owner=%d }".formatted(id, state, ownerId);
        }
    }
